...
public class LoginController {
    ...
    public Object toAddStudent(Model model) {
    ...
    public Object AddStudent(Student student,Model model) {
        int flag = 0;
        AjaxResult result = new AjaxResult();
        List<String> accountList = studentService.getStudentAccount();
        if(accountList.contains(student.getStudentAccount())){
            result.setSuccess(false);
            flag = 1;
        } else {
            ValueOperations<String, String> forValue = studentService.getRt().opsForValue();
            String key = PRE+student.getStudentEmail();
            String registerNumber = forValue.get(key);
            if (!Objects.isNull(registerNumber) && student.getRegisterNumber().equals(registerNumber)) {
                result.setSuccess(true);
                studentService.AddStudent(student);
                studentService.getRt().delete(key);
            } else {
                flag = 2;
                result.setSuccess(false);
            }
