...
public class PaperController {
    ...
    public String RandomADDQuestion(@PathVariable("id") Integer papid, String questionCourse, int QuesNums) {
        //找到试卷所有未分配的试题
        List<Question> questionsNodivIds;
        ...
        if (("all").equals(questionCourse)) {
            questionsNodivIds = questionService.queryAllQueIdNotInPaperByIdIfAll(papIdQuesCourse);
        } else {
            questionsNodivIds = questionService.queryAllQueIdNotInPaperById(papIdQuesCourse);
        }
        List<Integer> list = new ArrayList<>();
        for (Question question : questionsNodivIds) {
            list.add(question.getQuestionId());
        }
        if (QuesNums > list.size()) {
            QuesNums = list.size();
        }
        if (QuesNums <= 0) {
            QuesNums = 1;
